...
/**
 * \fn INT faspxx_solver_pardiso (dCSRmat *ptrA, dvector *b, dvector *u,
 *                              const SHORT prtlvl)
 *
...
INT faspxx_solver_pardiso(dCSRmat* ptrA, dvector* b, dvector* u, const SHORT prtlvl)
{
    ...
    DBL*   f = b->val; /* RHS vector */
    DBL*   x = u->val; /* Solution vector */
...
#if WITH_PARDISO
/**
 * \fn INT faspxx_pardiso_factorize (dCSRmat *ptrA, Pardiso_data *pdata,
 *                                 const SHORT prtlvl)
...
INT faspxx_pardiso_factorize(dCSRmat* ptrA, Pardiso_data* pdata, const SHORT prtlvl)
{
...
#ifdef _OPENMP
    pdata->iparm[2] = faspxx_get_num_threads();
#endif
    ...
    if (prtlvl > PRINT_MIN) {
        faspxx_gettime(&end_time);
        DBL factoization_time = end_time - start_time;
...
/**
 * \fn INT faspxx_pardiso_solve (dCSRmat *ptrA, dvector *b, dvector *u,
...
/**
 * \fn INT faspxx_pardiso_free (Pardiso_data *pdata)
