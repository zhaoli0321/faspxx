...
BEGIN {
  ...
  print " *"
  print " *---------------------------------------------------------------------------------"
  print " *  Copyright (C) 2024--Present by the FASP++ team. All rights reserved.           "
...
!/^INT|^SHORT|^LONG|^DBL|^FILE|^OFF_T|^size_t|^off_t|^pid_t|^unsigned|^mode_t|^DIR|^user|^int|^short|^long|^char|^uint|^struct|^BOOL|^void|^double|^time|^dCSRmat|^dCOOmat|^dvector|^iCSRmat|^ivector|^AMG_data|^ILU_data|^dSTRmat|^dBSRmat|^dCSRLmat|^precond|^cudvector|^cuivector|^Mumps_data|^cudCSRmat/ {
  next;
}
